...
public class Logger
{
	...
	private static String packageName = AppApplication.getInstance().getPackageName();// 修改包名,
	private static int maxSize = 1000;// KB
	private static File folder = new File(AbConstant.DIR_COMP_LOGS);
	private static File save = new File(folder.getPath() + File.separator + "SaveString.txt");
	private static File log = new File(folder.getPath() + File.separator + "Log.txt");
	private static File temp = new File(folder.getPath() + File.separator + "Temp.txt");
	 ...
	 */
	public static void record(String tag,Object infos)
	{
		move = 2;
		out(CLogType.D, tag+"  "+infos.toString());
	...
	{
		Logger.showLogInfo = showLogInfo;
	}
