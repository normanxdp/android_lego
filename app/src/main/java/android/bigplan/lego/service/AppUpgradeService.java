package android.bigplan.lego.service;
...
import android.bigplan.lego.util.DownloadUtils;
...
public class AppUpgradeService extends Service{
    ...
    private static final String TAG = AppUpgradeThread.class.getSimpleName();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        /*if (Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
            destDir = new File(Environment.getExternalStorageDirectory().getPath() + AppApplication.mDownloadPath);
            if (destDir.exists()) {
    ...
    class AppUpgradeThread extends Thread{
        ...
        public void run() {
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
                if (destDir == null) {
                    destDir = new File(AbAppConfig.ROOT+AbAppConfig.DOWNLOAD_ROOT_DIR);
                }
                if (destDir.exists() || destDir.mkdirs()) {
                    destFile = new File(destDir.getPath() + "/" + URLEncoder.encode(mDownloadUrl));
                    AbAppUtil.commonHandler.post(new Runnable() {
						@Override
						public void run() {
